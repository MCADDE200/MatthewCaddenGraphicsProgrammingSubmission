...
void Shader::initialise(const std::string& vertFile, const std::string& fragFile)
{
	...
	shaders[0] = CreateAShader(LoadTheShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateAShader(LoadTheShader(fragFile), GL_FRAGMENT_SHADER); // create fragment shader
	...
	CheckForShaderError(shaderProgram, GL_LINK_STATUS, true, "Error: Shader program linking failed"); // cheack for error

	glValidateProgram(shaderProgram); //check the entire program is valid
	CheckForShaderError(shaderProgram, GL_VALIDATE_STATUS, true, "Error: Shader program not valid");
...
void Shader::initGeometryShaders(const std::string& vertFile, const std::string& fragFile, const std::string& geomFile)
{
	shaderProgram = glCreateProgram();
	shaders[0] = CreateAShader(LoadTheShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateAShader(LoadTheShader(fragFile), GL_FRAGMENT_SHADER); // create fragment shader
	shaders[2] = CreateAShader(LoadTheShader(geomFile), GL_GEOMETRY_SHADER); // create fragment shader
